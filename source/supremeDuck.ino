...
#define LOG_SERIAL true //setting to true makes mouse movement laggy if the serial monitor isn't open
...
{ 
  ...
  if(lastSendingTime - previousSendingTime > 1000)
  {
      previousSendingTime = lastSendingTime;
      char data[40];
      sprintf(data,"data=%i,%s,end", useMultiLangWindowsMethod,encodingName);
      BTSerial.write(data);
      for(byte i=0;i<40;i++){data[i]=0;}
  }
} 
